...
NodeToJavaUtils.prototype.toJavaBean = function (obj, cb) {
    var self = this;
    if (_.isArray(obj)) {
        return self.toJavaCollection(obj, cb);
    };

    if (!_.isObject(obj) || _.isDate(obj)) {
        return cb(null, self.cast(obj));
    }
    ...
    async.forEachOf(obj, function (item, k, callback) {
        self.toJavaBean(item, function (err, res) {
            // console.log('ToJavabean err, res',err, res)
            if (err) {
                return callback(err);
            }
            objValues.put(k, res, callback);
        });
    }, function (err) {
        if (err) {
            return cb(err);
        ...
        return cb(null, dynObj);
    });

}

NodeToJavaUtils.prototype.toJavaCollection = function (col, cb) {
    var self = this;
    var list = new this.cl.JavaList();
    async.eachSeries(col, function (obj, callback) {
        self.toJavaBean(obj, function(err, res){
            list.add(res, callback);
        })
    },function(err){
        cb(err, list);
    })
